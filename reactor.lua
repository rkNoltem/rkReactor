...
print([[rkReactor  Copyright (C) 2015  Sean Harris
...
while true do
  ...
  stats[1][2] = round(reactor.getCasingTemperature())
  stats[2][2] = round(reactor.getFuelTemperature())
  stats[3][2] = round(reactor.getFuelReactivity())
  stats[4][2] = round(100 * reactor.getFuelAmount() / reactor.getFuelAmountMax())
  
  if isCooled then
    stats[5][2] = round(100 * reactor.getCoolantAmount() / reactor.getCoolantAmountMax())
    stats[6][2] = round(100 * reactor.getHotFluidAmount() / reactor.getHotFluidAmountMax())
    stats[7][2] = round(reactor.getHotFluidProducedLastTick())
    stats[8][2] = round(rodLevel)
    stats[9][2] = isActive
  else
    stats[5][2] = round(reactor.getEnergyStored())
    stats[6][2] = round(reactor.getEnergyProducedLastTick())
    stats[7][2] = round(rodLevel)
    stats[8][2] = isActive
    stats[9][2] = isBackup  
  end
